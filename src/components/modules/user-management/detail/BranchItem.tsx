...
export function BranchItem({ branch }: { branch: any }) {
    ...
    <div className="bg-foreground overflow-hidden rounded-lg">
      ...
      >
        <div className="min-w-0 flex-1">
          <span className="text-contrast text-[12px] font-medium">
            {branch.type === 'main' ? '본사' : '종사업장'}
          </span>
          <span className="text-contrast/70 ml-1.5 text-[11px]">#{branch.allias}</span>
        </div>
        {branch.is_default && (
      ...
      {open && (
        <div className="border-border border-t px-3 pt-2.5 pb-3 text-[12px]">
          ...
          <InfoRow label="개업일" value={branch.start_dt && formatDate(branch.start_dt)} />
          <InfoRow label="등록일" value={branch.created_at && formatDate(branch.created_at)} />

          {branch.delivery_available_days !== null && (
            <div className="mt-2">
              <p className="text-contrast/50 mb-1.5">배송 가능 요일</p>
                  ...
                  return (
                    <span
                      ...
                      className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${
                        available ? 'bg-primary/10 text-primary' : 'bg-foreground text-contrast/40'
                      }`}
                    >
                      {name}
                    </span>
