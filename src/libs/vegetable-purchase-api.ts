...
const customAPI = createInstance(ERP_BASE_URL.Prod);

export const getAvailableDate = async (): Promise<any> => {
  const response = await customAPI.get(PATH.api.getVegetableAvailableDate);
...
export const getBatchVegetablePurchaseProductManual = async (
  ...
  const response = await customAPI.get(PATH.api.getBatchVegetablePurchaseProductManual, {
...
export const patchPurchase = async (data: any): Promise<any> => {
  const response = await customAPI.patch(PATH.api.patchBatchVegetablePurchaseProductManual, data);
  return response.data;
};
